...
This directory contains VHS tape files for generating reproducible GIFs for documentation.
...
This builds cupcake and generates all GIFs from tape files.
...
All paths in tape files are relative to the `assets/` directory (VHS runs from there).

```tape
# Source common settings (relative to assets/)
Source tapes/common/settings.tape

# Define GIF output (relative to assets/)
Output output/gifs/my-demo.gif
...
**Important**: VHS has issues with absolute paths and deep relative paths. All paths must be relative to the `assets/` directory where VHS runs from.
...
Or create a custom theme JSON:

```tape
Set Theme { "name": "Custom", "background": "#1a1a2e", "foreground": "#eaeaea", ... }
```

## Generated Assets

After running `just assets`, you'll have:
