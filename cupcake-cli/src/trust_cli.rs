...
impl TrustCommand {
    ...
    pub async fn execute(&self) -> Result<()> {
        match self {
            TrustCommand::Init { project_dir, empty } => trust_init(project_dir, *empty).await,
            TrustCommand::Update {
                project_dir,
                dry_run,
                yes,
            ...
            TrustCommand::Disable { project_dir } => trust_disable(project_dir).await,
            TrustCommand::Enable {
                project_dir,
                verify,
            } => trust_enable(project_dir, *verify).await,
            TrustCommand::Reset { project_dir, force } => trust_reset(project_dir, *force).await,
        }
    }
}
...
async fn trust_update(project_dir: &Path, dry_run: bool, auto_yes: bool) -> Result<()> {
                ...
                Some(existing_entry) => {
                    if existing_entry.hash() != current_entry.hash() {
                        modified.push((
                            category.clone(),
                            name.clone(),
                            existing_entry.hash().to_string(),
                            current_entry.hash().to_string(),
                        ));
    ...
    for (category, manifest_category_scripts) in manifest.scripts() {
        for name in manifest_category_scripts.keys() {
            if !current_scripts
                .get(category)
    ...
    for (category, name, old_hash, new_hash) in &modified {
        println!("  ~ {category}/{name} (modified)");
...
async fn trust_verify(project_dir: &Path, verbose: bool) -> Result<()> {
    ...
    let manifest = TrustManifest::load(&trust_file).context("Failed to load trust manifest")?;

    let total_scripts: usize = manifest.scripts().values().map(|s| s.len()).sum();
    println!("📋 Checking {total_scripts} scripts in manifest...\n");
    ...
    for (category, scripts) in manifest.scripts() {
        if verbose {
            println!("📁 Verifying {category} scripts:");
        }
    ...
    println!("   ✅ Passed:   {passed} scripts");
    if failed > 0 {
        println!("   ❌ Modified: {failed} scripts");
    }
    if missing > 0 {
        println!("   ⚠️  Missing:  {missing} scripts");
    }
...
async fn trust_list(project_dir: &Path, show_modified: bool, show_hashes: bool) -> Result<()> {
    ...
    if show_modified && (modified_count > 0 || missing_count > 0) {
        println!("\n📊 Status Summary:");
        if modified_count > 0 {
            println!("   ❌ {modified_count} modified scripts");
        }
        if missing_count > 0 {
            println!("   ⚠️  {missing_count} missing scripts");
        }
        println!("   Run 'cupcake trust update' to approve changes");
    }
...
async fn trust_disable(project_dir: &Path) -> Result<()> {
    ...
    let mut manifest = TrustManifest::load(&trust_file).context("Failed to load trust manifest")?;
...
async fn trust_reset(project_dir: &Path, force: bool) -> Result<()> {
    ...
    if !force {
        println!("⚠️  This will delete the trust manifest and disable integrity verification.");
        println!("   All script approvals will be lost.");
        print!("Continue? [y/N]: ");
        io::stdout().flush()?;
    ...
    std::fs::remove_file(&trust_file).context("Failed to remove trust manifest")?;
...
mod tests {
    ...
    fn test_command_availability() {
        ...
        let commands = vec![
            "init", "update", "verify", "list", "disable", "enable", "reset",
        ];
        ...
        for cmd_name in commands {
            ...
            let cmd = match cmd_name {
                "init" => TrustCommand::Init {
                    project_dir,
                    empty: false,
                },
                "update" => TrustCommand::Update {
                    project_dir,
                    dry_run: false,
                    yes: false,
                },
                "verify" => TrustCommand::Verify {
                    project_dir,
                    verbose: false,
                },
                "list" => TrustCommand::List {
                    project_dir,
                    hashes: false,
                    modified: false,
                },
                "disable" => TrustCommand::Disable { project_dir },
                "enable" => TrustCommand::Enable {
                    project_dir,
                    verify: false,
                },
                "reset" => TrustCommand::Reset {
                    project_dir,
                    force: false,
                },
