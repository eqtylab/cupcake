...
"#;
    ...
    assert!(scripts_seen.contains("signal:explicit_signal"), "Should see explicit signal");
    assert!(scripts_seen.contains("signal:auto_signal"), "Should see auto-discovered signal");
    assert!(scripts_seen.contains("action:on_any_denial"), "Should see on_any_denial");
    assert!(scripts_seen.contains("action:RULE-001"), "Should see RULE-001");
    assert!(scripts_seen.contains("action:RULE-002"), "Should see auto-discovered RULE-002");
    
    println!("✅ Trust CLI (after fix) correctly sees all {} scripts", scripts_seen.len());
