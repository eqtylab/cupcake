...
mod integration_tests {
    ...
    async fn test_debug_capture_with_allow_decision() {
        ...
        let _decision = engine.evaluate(&event, debug_capture.as_mut()).await.unwrap();
        ...
        assert!(matches!(debug.final_decision.unwrap(), crate::engine::decision::FinalDecision::Allow { .. }));
    ...
    async fn test_no_debug_when_disabled() {
        ...
        assert!(matches!(decision, crate::engine::decision::FinalDecision::Allow { .. }));
        ...
        if debug_dir.exists() {
            let entries: Vec<_> = std::fs::read_dir(debug_dir)
                ...
                .filter(|e| {
                    e.file_name().to_string_lossy().contains("test-no-debug")
                })
                .collect();
            assert!(entries.is_empty(), "No debug files should be created when disabled");
        }
    }
...
mod performance_tests {
    ...
    fn test_zero_overhead_when_disabled() {
        ...
        assert!(disabled_duration.as_millis() < 10, 
