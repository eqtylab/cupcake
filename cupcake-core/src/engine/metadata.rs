...
pub fn validate_routing_directive(directive: &RoutingDirective, package_name: &str) -> Result<()> {
    let is_system_policy =
        package_name.starts_with("cupcake.system") || package_name.starts_with("governance.system");
    ...
    if is_system_policy {
        ...
        debug!(
            "System policy {} has valid empty routing directive",
            package_name
        );
        return Ok(());
    }
    ...
    for event in &directive.required_events {
        if !valid_events.contains(&event.as_str()) {
            warn!(
                "Unknown event type '{}' in routing directive for policy '{}'. Known events: {:?}",
    ...
    debug!(
        "Regular policy {} has valid routing directive",
        package_name
    );
    Ok(())
}
...
"#;
    ...
    fn test_validate_routing_directive() {
        ...
        assert!(
            validate_routing_directive(&invalid_regular, "cupcake.policies.bash_guard").is_err()
        );
        ...
        assert!(
            validate_routing_directive(&valid_empty_regular, "cupcake.policies.global_rules")
                .is_ok()
        );
    }
