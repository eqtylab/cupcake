...
pub fn compute_hmac(message: &[u8], project_path: &Path) -> Result<String> {
    ...
    let mut mac = HmacSha256::new_from_slice(&key).context("Failed to create HMAC instance")?;
...
pub fn verify_hmac(message: &[u8], signature: &str, project_path: &Path) -> Result<bool> {
    ...
    let signature_bytes = hex::decode(hex_signature).context("Failed to decode HMAC signature")?;
