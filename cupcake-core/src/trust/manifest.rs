...
use crate::trust::hasher::{compute_hmac, hash_file, hash_file_sync, hash_string, verify_hmac};
use anyhow::Result;
...
impl ScriptReference {
    ...
    pub fn compute_hash_sync(&self) -> Result<String> {
        match self {
            ScriptReference::Inline(cmd) => Ok(hash_string(cmd)),
            ScriptReference::File(path) => hash_file_sync(path),
            ScriptReference::Complex { script_path, .. } => hash_file_sync(script_path),
        }
    }
...
impl TrustManifest {
    ...
    pub fn load(manifest_path: &Path) -> Result<Self, TrustError> {
        ...
        let content =
            std::fs::read_to_string(manifest_path).map_err(|e| TrustError::ManifestReadError {
    ...
    pub fn save(&mut self, manifest_path: &Path) -> Result<(), TrustError> {
        ...
        // Compute HMAC
        let hmac = compute_hmac(json_content.as_bytes(), project_path).map_err(|_| {
            TrustError::ManifestWriteError {
                path: manifest_path.to_path_buf(),
                source: std::io::Error::other("Failed to compute HMAC"),
            }
        })?;

        // Write with HMAC appended
        let final_content = format!("{json_content}\n\n/* HMAC: {hmac} */");
        ...
        std::fs::write(manifest_path, final_content).map_err(|e| {
            TrustError::ManifestWriteError {
                path: manifest_path.to_path_buf(),
                source: e,
            }
