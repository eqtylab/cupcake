...
use crate::trust::manifest::{TrustManifest, ScriptReference};
use anyhow::Result;
...
impl TrustVerifier {
    ...
    pub async fn verify_script(&self, command: &str) -> Result<(), TrustError> {
        ...
        let current_hash = script_ref.compute_hash().await
            .map_err(|e| {
...
mod tests {
    ...
    async fn test_verify_trusted_inline_script() {
        ...
        manifest.add_script("signals", "test", ScriptEntry {
