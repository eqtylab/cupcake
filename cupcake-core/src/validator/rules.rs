...
use super::{ValidationRule, ValidationIssue, PolicyContent, Severity};
use regex::Regex;
...
impl ValidationRule for ObjectKeyMembershipRule {
    ...
    fn check(&self, policy: &PolicyContent) -> Vec<ValidationIssue> {
        ...
        let object_membership_pattern = Regex::new(r#""[^"]+"\s+in\s+[a-zA-Z_][a-zA-Z0-9_\.]*"#).unwrap();
        ...
        for (i, line) in policy.lines.iter().enumerate() {
            if object_membership_pattern.is_match(line) {
                issues.push(ValidationIssue {
                    ...
                    rule_id: self.rule_id(),
                    message: "Consider using object.keys() for object key membership in Rego v1".to_string(),
                    line: Some(i + 1),
                    });
            }
        }
        ...
        if let Some(metadata) = &policy.metadata {
            if metadata.custom.routing.is_none() {
                    issues.push(ValidationIssue {
                        severity: Severity::Warning,
                        rule_id: self.rule_id(),
                        message: "Policy should have routing metadata (required_events, required_tools)".to_string(),
                        line: None,
                            });
                }
        } else {
            ...
            if locations.len() > 1 {
                ...
                if !is_grouped {
                    issues.push(ValidationIssue {
                        severity: Severity::Warning,
                        rule_id: self.rule_id(),
                        message: format!("Multiple '{}' rules should be grouped together", rule_name),
