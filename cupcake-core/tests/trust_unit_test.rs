...
use anyhow::Result;
use cupcake_core::trust::manifest::ScriptReference;
use cupcake_core::trust::{TrustError, TrustManifest};
use std::fs;
...
async fn test_script_reference_parsing() -> Result<()> {
    ...
    assert!(matches!(
        complex_ref,
        ScriptReference::Inline(_) | ScriptReference::Complex { .. }
    ));
...
async fn test_trust_manifest_script_operations() -> Result<()> {
    ...
    let script_entry =
        cupcake_core::trust::manifest::ScriptEntry::from_command("echo test", temp_dir.path())
            .await?;
...
async fn test_trust_manifest_round_trip_with_scripts() -> Result<()> {
    ...
    let script_entry = cupcake_core::trust::manifest::ScriptEntry::from_command(
        "echo hello world",
        temp_dir.path(),
    )
    .await?;
