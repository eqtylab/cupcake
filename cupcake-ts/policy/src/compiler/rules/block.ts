/**
 * Compile BlockRule to Rego.
 */

import type { BlockRule } from '../../core/index';
...
/**
 * Compiles a block rule to Rego.
 *
 * Block rules are used for:
 ...
 */
export function compileBlockRule(rule: BlockRule, ctx: CompilerContext): string {
  const lines: string[] = [];

  lines.push('block contains decision if {');

  // Hook event check (required)
  lines.push(indent(`input.hook_event_name == "${rule.event}"`, 1));

  // Tool check (only for PostToolUse with tools)
