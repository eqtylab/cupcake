...
This is the Cursor-specific version that handles:
- tool_name = "execute_sql" (not mcp__postgres__execute_sql)
- tool_input as JSON string (not object)
"""
import json
import sys
import re
...
def main():
    try:
        ...
        # Cursor uses "execute_sql" as tool name (not mcp__postgres__execute_sql)
        if input_data.get("tool_name") != "execute_sql":
            print(json.dumps({"relevant": False}))
            return

        # Cursor sends tool_input as JSON string, need to parse it
        ...
        # Extract identifier from original SQL (not lowercased)
        original_sql = tool_input.get("sql", "")
        identifier = extract_identifier(original_sql)
        if not identifier:
        ...
        print(json.dumps({"error": str(e), "relevant": False}))
