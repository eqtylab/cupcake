...
pub async fn load_preview(path: &Path) -> Result<String> {
    ...
    tokio::task::spawn_blocking(move || load_preview_sync(&path))
        .await
...
pub fn mock_preview(path: &Path) -> String {
    match path.to_str() {
        Some("CLAUDE.md") => r#"# Claude Development Rules
...
- Use parameterized queries"#
            .to_string(),
        Some(".cursor/rules") => r#"# Cursor AI Rules
...
5. Use meaningful variable names"#
            .to_string(),
        Some(".aider.conf.yml") => r#"# Aider configuration
...
watch:
  ...
  - "Cargo.toml""#
            .to_string(),
        _ if path.to_str().unwrap_or("").contains("windsurf") => r#"# Windsurf Rule
