...
fn render_simple(frame: &mut Frame, area: Rect, state: &LandingState) {
    ...
    // Description
    lines.push(Line::from(
        "Turn your rules into enforced policies and performance improvements.",
    ));
    lines.push(Line::from(
        "Cupcake will auto-create policies and hooks from your existing rules.",
    ));
    lines.push(Line::from(vec![
        Span::styled(
            "You decide which hooks to keep",
            Style::default().fg(Color::Green),
        ),
        Span::raw(". "),
        Span::styled(
            "You can also use an intelligent rules/hook builder from scratch",
    ...
    let ascii_paragraph = Paragraph::new(ascii_lines).alignment(Alignment::Left);
    frame.render_widget(ascii_paragraph, ascii_area);

    // Render text content with center alignment
    let text_paragraph = Paragraph::new(text_lines).alignment(Alignment::Center);
