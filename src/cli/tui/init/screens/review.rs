...
use crate::cli::tui::init::state::{ExtractedRule, ReviewState, Severity};
use ratatui::{
...
pub fn render(frame: &mut Frame, state: &ReviewState) {
    ...
    let main_block = Block::default()
        .title(format!(
            " Review Extracted Rules ({} of {} selected) ",
            state.selected.len(),
            state.rules.len()
        ))
        .borders(Borders::ALL);
...
fn render_header(frame: &mut Frame, area: Rect, _state: &ReviewState) {
    let content = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                "Selected rules will be converted to Cupcake policies.",
...
fn render_rule_table(frame: &mut Frame, area: Rect, state: &ReviewState) {
    ...
    let mut sorted_rules: Vec<(usize, &ExtractedRule)> = state.rules.iter().enumerate().collect();
    ...
    // Table rows
    let rows: Vec<Row> = sorted_rules
        .iter()
        .enumerate()
        .map(|(display_idx, (actual_idx, rule))| {
            ...
            let rule_desc = if rule.description.len() > rule_truncate_len {
                format!(
                    "{}...",
                    &rule.description[..rule_truncate_len.saturating_sub(3)]
                )
            } else {
            ...
            let hook_desc = if rule.hook_description.len() > hook_truncate_len {
                format!(
                    "{}...",
                    &rule.hook_description[..hook_truncate_len.saturating_sub(3)]
                )
            } else {
            ...
            let rationale = if rule.policy_decision.rationale.len() > rationale_truncate_len {
                format!(
                    "{}...",
                    &rule.policy_decision.rationale[..rationale_truncate_len.saturating_sub(3)]
                )
            } else {
...
fn render_help(frame: &mut Frame, area: Rect) {
    ...
    let help = Paragraph::new(help_text).style(Style::default().bg(Color::DarkGray));

    frame.render_widget(help, area);
}

#[allow(clippy::vec_init_then_push)]
// Rationale: Complex TUI composition is clearer as stepwise pushes.
// Converting to a single `vec![...]` harms readability and maintainability.
