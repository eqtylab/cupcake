...
pub fn render(frame: &mut Frame, state: &SuccessState) {
    ...
    let success_paragraph = Paragraph::new(success_text).alignment(Alignment::Left);
...
fn render_summary_table(frame: &mut Frame, area: Rect, state: &SuccessState) {
    ...
    let rows = vec![
        ...
        Row::new(vec![
            ...
            Cell::from(format!(
                "{} rules - important policy enforcement",
                state.high_count
            ))
        ...
        Row::new(vec![
            ...
            Cell::from(format!(
                "{} rules - informational guidelines",
                state.low_count
            ))
        ...
        Row::new(vec![
            ...
            Cell::from("~8ms average per file operation").style(Style::default().fg(Color::Green)),
        ]),
        Row::new(vec![
    ...
    let table = Table::new(rows, vec![Constraint::Length(22), Constraint::Min(40)])
        .block(
            Block::default()
...
fn render_try_commands(frame: &mut Frame, area: Rect) {
    let commands = vec![Line::from("  Try these commands:"), Line::from("")];
