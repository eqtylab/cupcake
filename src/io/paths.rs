...
impl CupcakePaths {
    ...
    pub fn new() -> Result<Self> {
        let dirs = ProjectDirs::from("", "", "cupcake")
            .ok_or_else(|| CupcakeError::Path("Failed to determine project directories".to_string()))?;
    ...
    pub fn policy_cache_file(&self, project_root: &std::path::Path) -> PathBuf {
        self.cache_dir.join(format!("policy-{}.cache", 
    ...
    pub fn default_claude_settings() -> Result<PathBuf> {
        let dirs = ProjectDirs::from("", "", "claude")
            .ok_or_else(|| CupcakeError::Path("Failed to determine Claude directories".to_string()))?;
...
mod tests {
    ...
    fn test_policy_file_paths() {
        ...
        assert_eq!(paths.policy_file(project_root), project_root.join("cupcake.toml"));
    ...
    fn test_path_utils_safety() {
        ...
        assert!(!PathUtils::is_safe_path(Path::new("safe/../unsafe/file.txt")));
    }
