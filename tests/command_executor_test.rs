...
use cupcake::engine::command_executor::{CommandExecutor, ExecutionError, Operation};
...
mod command_executor_tests {
    ...
    fn test_template_substitution_in_command() {
        ...
        assert_eq!(
            node.command.env_vars.get("SESSION_ID"),
            Some(&"sess-12345".to_string())
        );
    }
    ...
    fn test_redirect_operations_construction() {
        ...
        assert_eq!(
            node.command.env_vars.get("RUSTFLAGS"),
    ...
    fn test_conditional_execution_construction() {
        ...
        assert_eq!(
            conditional.on_success[0].command.args,
            vec!["File /tmp/test.txt exists"]
        );
        assert_eq!(conditional.on_success[1].command.program, "wc");
        assert_eq!(
            conditional.on_success[1].command.args,
            vec!["-l", "/tmp/test.txt"]
        );
        ...
        assert_eq!(
            ...
            vec!["File /tmp/test.txt not found"]
        );
    }
