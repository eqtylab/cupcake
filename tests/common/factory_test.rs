...
fn test_event_factory_produces_valid_json() {
    ...
    let parsed: Value =
        serde_json::from_str(&pre_tool_json).expect("PreToolUse should produce valid JSON");
    ...
    let parsed: Value =
        serde_json::from_str(&post_tool_json).expect("PostToolUse should produce valid JSON");
    ...
    let parsed: Value =
        serde_json::from_str(&prompt_json).expect("UserPromptSubmit should produce valid JSON");
    ...
    let parsed: Value =
        serde_json::from_str(&compact_json).expect("PreCompact should produce valid JSON");
