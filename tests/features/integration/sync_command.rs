...
fn test_sync_generates_correct_json_structure() {
    ...
    let settings_content =
        fs::read_to_string(&settings_path).expect("Should be able to read settings file");

    let settings: Value =
        serde_json::from_str(&settings_content).expect("Settings file should contain valid JSON");
    ...
    let pre_tool_use = hooks
        .get("PreToolUse")
        .expect("Should have PreToolUse hook");
    let pre_tool_use_array = pre_tool_use
        .as_array()
        .expect("PreToolUse should be an array");
    assert_eq!(
        pre_tool_use_array.len(),
        1,
        "PreToolUse should have one matcher"
    );

    let matcher = &pre_tool_use_array[0];
    assert_eq!(matcher["matcher"], "*", "Should have wildcard matcher");

    let hooks_array = matcher["hooks"]
        .as_array()
        .expect("Should have hooks array");
    ...
    assert_eq!(
        hook_cmd["command"], "cupcake run --event PreToolUse",
        "Should have correct command format"
    );
    ...
    let user_prompt_submit = hooks
        ...
        .expect("Should have UserPromptSubmit hook");
    let user_prompt_submit_array = user_prompt_submit
        .as_array()
        .expect("UserPromptSubmit should be an array");
    assert_eq!(
        user_prompt_submit_array.len(),
        1,
        "UserPromptSubmit should have one entry"
    );

    let ups_entry = &user_prompt_submit_array[0];
    assert!(
        ups_entry.get("matcher").is_none(),
        "UserPromptSubmit should not have matcher"
    );

    let ups_hooks = ups_entry["hooks"]
        .as_array()
        .expect("Should have hooks array");
    let ups_hook_cmd = &ups_hooks[0];
    assert_eq!(
        ups_hook_cmd["command"], "cupcake run --event UserPromptSubmit",
        "Should have correct UPS command"
    );
    ...
    for event in &expected_events {
        ...
        for entry in event_hooks {
            ...
            for hook_cmd in entry_hooks {
                assert_eq!(
                    hook_cmd["type"], "command",
                    "All hooks should be command type"
                );
                assert!(
                    hook_cmd["command"]
                        .as_str()
                        .unwrap()
                        .starts_with("cupcake run --event"),
                    "All commands should use --event format"
                );
                assert!(
                    hook_cmd["timeout"].is_number(),
                    "All hooks should have numeric timeout"
                );
            }
        }
    }
...
fn test_sync_preserves_existing_user_settings() {
    ...
    let updated_content = fs::read_to_string(&settings_path).expect("Should read updated settings");
    let updated_settings: Value =
        serde_json::from_str(&updated_content).expect("Updated settings should be valid JSON");

    // Verify user settings are preserved
    assert_eq!(
        updated_settings["model"], "claude-3-5-sonnet-20241022",
        "Should preserve model setting"
    );
    assert_eq!(
        updated_settings["customInstructions"], "Always be helpful",
        "Should preserve custom instructions"
    );

    // Verify existing hooks are preserved (not overwritten without --force)
    let hooks = updated_settings["hooks"]
        .as_object()
        .expect("Should have hooks");
    let pre_tool_use = hooks["PreToolUse"]
        .as_array()
...
fn test_sync_force_mode_overwrites_hooks() {
            ...
            .map(|hooks_array| {
                hooks_array.iter().any(|cmd| {
                    cmd.get("command").and_then(|c| c.as_str())
                        == Some("cupcake run --event PreToolUse")
                })
            })
            .unwrap_or(false)
    });
    assert!(
        cupcake_hook_exists,
        "Should have Cupcake's PreToolUse hook after --force"
    );
}
...
fn test_sync_dry_run_mode() {
    ...
    assert!(
        ...
        "Settings file should not be created in dry-run mode"
    );
