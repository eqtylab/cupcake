...
"#;
    ...
    assert!(response_json.get("continue").is_none(), "Should not have 'continue' field");
    assert!(response_json.get("stopReason").is_none(), "Should not have 'stopReason' field");
}
...
"#;
    ...
    assert!(stderr.contains("Ask action not supported for UserPromptSubmit") ||
    ...
    assert_eq!(hook_output["additionalContext"], "Please confirm this action");
...
"#;
    ...
    let cupcake_binary = env!("CARGO_BIN_EXE_cupcake");
    let mut child = Command::new(cupcake_binary)
        .args(["run", "--event", "PreToolUse", "--config", guardrails_dir.join("cupcake.yaml").to_str().unwrap()])
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
    ...
    child.stdin.as_mut().unwrap().write_all(edit_event.as_bytes()).unwrap();
    let output = child.wait_with_output().unwrap();
    ...
    let response_json: serde_json::Value = serde_json::from_str(&stdout).expect("stdout should be valid JSON");
    ...
    child2.stdin.as_mut().unwrap().write_all(editfile_event.as_bytes()).unwrap();
    ...
    // For PreToolUse, provide_feedback doesn't output to stdout/stderr
    let response_json2: serde_json::Value = serde_json::from_str(&stdout2).expect("stdout should be valid JSON");
    assert_eq!(response_json2["hookSpecificOutput"]["permissionDecision"], "allow");
...
"#;
    ...
    assert!(!stdout.contains("hookSpecificOutput"), "Should be raw text, not JSON");
}