...
"#;
    ...
    child.stdin.as_mut().unwrap().write_all(bash_event.as_bytes()).unwrap();
    ...
    child2.stdin.as_mut().unwrap().write_all(bashscript_event.as_bytes()).unwrap();
...
"#;
    ...
    child2.stdin.as_mut().unwrap().write_all(edit_event.as_bytes()).unwrap();
    let output2 = child2.wait_with_output().unwrap();
    ...
    child3.stdin.as_mut().unwrap().write_all(read_event.as_bytes()).unwrap();
