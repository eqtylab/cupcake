...
fn test_sync_do_no_harm() {
    ...
    fs::write(&settings_path, serde_json::to_string_pretty(&user_settings).unwrap()).unwrap();
    ...
    assert!(user_bash_hook_exists, "User's Bash hook should be preserved");
    
    let userprompt_hooks = json_after_first["hooks"]["UserPromptSubmit"].as_array().unwrap();
    ...
    assert!(user_prompt_hook_exists, "User's prompt hook should be preserved");
    
    // 2. Other settings are preserved
    assert_eq!(
        json_after_first["model"],
        "claude-3-opus-20240229",
        "Model setting should be preserved"
    );
    assert_eq!(
        json_after_first["permissions"]["api_access"],
        true,
        "Permissions should be preserved"
    );
    assert_eq!(
        json_after_first["custom_field"],
        "should_be_preserved",
        "Custom fields should be preserved"
    );
    
    // 3. Cupcake hooks are present
    let cupcake_pretool_exists = pretooluse_hooks.iter().any(|hook| {
        hook.get("managed_by").and_then(|v| v.as_str()) == Some("cupcake")
    });
    assert!(cupcake_pretool_exists, "Cupcake PreToolUse hooks should be added");
    
    let cupcake_prompt_exists = userprompt_hooks.iter().any(|hook| {
        hook.get("managed_by").and_then(|v| v.as_str()) == Some("cupcake")
    });
    assert!(cupcake_prompt_exists, "Cupcake UserPromptSubmit hooks should be added");
...
fn test_sync_remove_then_append() {
    ...
    let settings_path = claude_dir.join("settings.local.json");
    fs::write(&settings_path, serde_json::to_string_pretty(&old_settings).unwrap()).unwrap();
    ...
    let new_cupcake_exists = pretooluse_hooks.iter().any(|hook| {
        hook.get("managed_by").and_then(|v| v.as_str()) == Some("cupcake") &&
        !hook["hooks"][0]["command"].as_str().unwrap().contains("--old-flag")
    });
