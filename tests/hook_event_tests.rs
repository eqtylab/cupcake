...
fn test_notification_event() {
    ...
    match event {
        HookEvent::Notification(payload) => {
            assert_eq!(payload.common.session_id, "test-session-789");
            assert_eq!(payload.message, "Claude needs permission to use the Bash tool");
        }
...
fn test_subagent_stop_event() {
    ...
    match event {
        HookEvent::SubagentStop(payload) => {
...
fn test_user_prompt_submit_event() {
    ...
    let event: HookEvent = serde_json::from_str(&prompt_json)
        .expect("Failed to deserialize UserPromptSubmit event");
...
fn test_session_start_event() {
    ...
    let event: HookEvent = serde_json::from_str(&session_json)
        .expect("Failed to deserialize SessionStart event");
