use cupcake::engine::events::{HookEvent, CommonEventData, CompactTrigger, BashToolInput, ReadToolInput};
use serde_json;
...
fn test_hook_event_deserialization() {
    ...
    let event: HookEvent = serde_json::from_str(pre_tool_use_json).expect("Failed to deserialize PreToolUse event");
    ...
    match event {
        HookEvent::PreToolUse { common, tool_name, tool_input } => {
            ...
            assert_eq!(common.hook_event_name, "PreToolUse");
...
fn test_post_tool_use_event() {
    ...
    let event: HookEvent = serde_json::from_str(post_tool_use_json).expect("Failed to deserialize PostToolUse event");
    
    match event {
        HookEvent::PostToolUse { common, tool_name, tool_input, tool_response } => {
...
fn test_notification_event() {
    ...
    let event: HookEvent = serde_json::from_str(notification_json).expect("Failed to deserialize Notification event");
...
fn test_stop_event() {
    ...
    let event: HookEvent = serde_json::from_str(stop_json).expect("Failed to deserialize Stop event");
...
fn test_subagent_stop_event() {
    ...
    let event: HookEvent = serde_json::from_str(subagent_stop_json).expect("Failed to deserialize SubagentStop event");
...
fn test_pre_compact_event() {
    ...
    let event: HookEvent = serde_json::from_str(pre_compact_json).expect("Failed to deserialize PreCompact event");
    ...
    match event {
        HookEvent::PreCompact { common, trigger, custom_instructions } => {
            ...
            assert_eq!(custom_instructions, Some("Save important context".to_string()));
        }
...
fn test_pre_compact_auto_trigger() {
    ...
    let event: HookEvent = serde_json::from_str(pre_compact_auto_json).expect("Failed to deserialize PreCompact auto event");
...
fn test_hook_event_helper_methods() {
    ...
    let event: HookEvent = serde_json::from_str(pre_tool_use_json).expect("Failed to deserialize event");
    ...
    let notification_event: HookEvent = serde_json::from_str(notification_json).expect("Failed to deserialize notification");
    ...
    let stop_event: HookEvent = serde_json::from_str(stop_json).expect("Failed to deserialize stop event");
...
fn test_tool_input_parsing() {
    ...
    let event: HookEvent = serde_json::from_str(bash_event_json).expect("Failed to deserialize Bash event");
    
    let bash_input: BashToolInput = event.parse_tool_input().expect("Failed to parse Bash tool input");
    ...
    let read_event: HookEvent = serde_json::from_str(read_event_json).expect("Failed to deserialize Read event");
    
    let read_input: ReadToolInput = read_event.parse_tool_input().expect("Failed to parse Read tool input");
...
fn test_compact_trigger_serialization() {
    ...
    let deserialized: CompactTrigger = serde_json::from_str(&json).expect("Failed to deserialize manual trigger");
    ...
    let deserialized: CompactTrigger = serde_json::from_str(&json).expect("Failed to deserialize auto trigger");
...
fn test_common_event_data() {
    ...
    let deserialized: CommonEventData = serde_json::from_str(&json).expect("Failed to deserialize common event data");
    ...
    assert_eq!(deserialized.hook_event_name, common_data.hook_event_name);
}