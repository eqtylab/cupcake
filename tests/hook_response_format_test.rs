...
"#;
    ...
    assert!(
        ...
        "PreToolUse must have hookSpecificOutput"
    );
    assert_eq!(
...
fn test_stop_response_format() {
    ...
    assert!(
        response.get("hookSpecificOutput").is_none(),
        "Stop must NOT have hookSpecificOutput"
    );
    assert!(
        response.get("continue").is_none(),
        "Allow should not set continue field"
    );
    assert!(
        response.get("stopReason").is_none(),
        "Allow should not set stopReason"
    );
}
...
fn test_notification_response_format() {
    ...
    assert!(
        response.get("hookSpecificOutput").is_none(),
        "Notification must NOT have hookSpecificOutput"
    );
...
fn test_precompact_response_format() {
    ...
    assert!(
        response.get("hookSpecificOutput").is_none(),
        "PreCompact must NOT have hookSpecificOutput"
    );
    assert!(response.get("continue").is_none());
    assert!(response.get("stopReason").is_none());
}
...
fn test_subagent_stop_response_format() {
    ...
    assert!(
        response.get("hookSpecificOutput").is_none(),
        "SubagentStop must NOT have hookSpecificOutput"
    );
    assert!(response.get("continue").is_none());
    assert!(response.get("stopReason").is_none());
}
