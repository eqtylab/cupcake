...
use cupcake_rego::trust::TrustManifest;
use std::fs;
...
async fn test_engine_without_trust() -> Result<()> {
    ...
    assert!(matches!(decision, cupcake_rego::engine::decision::FinalDecision::Allow { .. }));
...
async fn test_engine_with_valid_trust() -> Result<()> {
    ...
    let signal_entry = cupcake_rego::trust::manifest::ScriptEntry {
        script_type: "inline".to_string(),
        command: "echo 'test output'".to_string(),
        hash: cupcake_rego::trust::hasher::hash_string("echo 'test output'"),
        absolute_path: None,
        size: None,
        modified: None,
        interpreter: None,
